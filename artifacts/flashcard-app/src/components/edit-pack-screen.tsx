...
export function EditPackScreen({
  ...
  const handleSaveName = () => {
    if (!name.trim() || name.trim() === initialPackName) {
      setNameSaved(true);
      return;
    }
  ...
  const handleBulkImport = async () => {
    ...
    if (parsed.length === 0) {
      toast({
        title: "Không tìm thấy từ hợp lệ",
        description:
          'Mỗi dòng cần có từ và nghĩa, cách nhau bằng tab, " - ", hoặc dấu phẩy',
        variant: "destructive",
      });
      return;
    }
  ...
  const handleStartEdit = (word: VocabWord) => {
    ...
    setEditPinyin((word as any).pinyin ?? toPinyin(word.term));
 ...
 const handleToggleStar = async (word: VocabWord) => {
  try {
    const nextStarred = !(word as any).starred;
  ...
  const exportCsv = () => {
    ...
    const rows = isChinesePack
      ...
      : [
          ["STT", "Từ", "Nghĩa"],
          ...words.map((w, i) => [i + 1, w.term, w.meaning]),
        ];
        ...
        {showBulk && (
          <div className="px-5 pb-5 flex flex-col gap-3 border-t border-border">
            <p className="text-xs text-muted-foreground pt-3">
              Mỗi dòng một từ. Phân cách bằng <strong>Tab</strong> từ
              Excel/Sheets, <strong>" - "</strong>, hoặc dấu{" "}
              <strong>phẩy</strong>.
              <br />
              Ví dụ:{" "}
              <code className="bg-muted px-1 rounded">踢{"\t"}đá</code> hoặc{" "}
              <code className="bg-muted px-1 rounded">踢 - đá</code>
            </p>
                  ...
                  <div className="flex items-center gap-3 px-5 py-3 text-sm">
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <button
                        ...
                        data-testid={`button-star-word-${w.id}`}
                      >
                        <Star
                          className={`h-4 w-4 ${
                            (w as any).starred
