...
export function StudyScreen({
  ...
  packName,
  packLanguage,
  selectedWords: preselectedWords,
  onBack,
  onFinish,
}: StudyScreenProps) {
  ...
  const displayWord =
    mode === "word_to_meaning" ? currentWord?.term : currentWord?.meaning;
  ...
  useEffect(() => {
    const handleSpeakShortcut = (e: KeyboardEvent) => {
    ...
    window.addEventListener("keydown", handleSpeakShortcut);

    return () => {
      window.removeEventListener("keydown", handleSpeakShortcut);
    };
  }, [currentWord, packLanguage]);
...
speakWord(currentWord);
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            },
            {
              ...
              sub: isChinese
                ? "Nhìn nghĩa tiếng Việt, viết chữ Hán"
                : "Nhìn nghĩa, viết từ",
            },
          ].map((opt) => (
            <button
  ...
  return (
    <div className="w-full max-w-xl mx-auto flex flex-col gap-4">
      ...
      {currentWord && (
        <div
          className={`bg-card border rounded-lg p-6 flex flex-col gap-4 transition-colors ${
            feedback === "correct"
