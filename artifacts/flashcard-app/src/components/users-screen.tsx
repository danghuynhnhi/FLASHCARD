...
export function UsersScreen({ users, createUser, renameUser, deleteUser, onSelectUser }: UsersScreenProps) {
  ...
  const handleCreate = (e: React.FormEvent) => {
    ...
    if (createUser(newUserName)) {
      setNewUserName('');
    } else {
      toast({ title: 'Lỗi', description: 'Tên người dùng đã tồn tại hoặc không hợp lệ', variant: 'destructive' });
    }
  };

  const handleRename = () => {
    if (!editingUser) return;
    if (renameUser(editingUser.oldName, editingUser.newName)) {
      setEditingUser(null);
    } else {
      toast({ title: 'Lỗi', description: 'Tên người dùng đã tồn tại hoặc không hợp lệ', variant: 'destructive' });
    }
  ...
  return (
    <div className="w-full flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <Card className="bg-card/50 backdrop-blur-sm shadow-sm border-muted">
        <CardContent className="pt-6">
          <form onSubmit={handleCreate} className="flex gap-3">
            ...
            <Button type="submit" className="h-12 px-6 gap-2" data-testid="button-create-user">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {users.map(user => (
          <Card key={user.name} className="group hover:shadow-md transition-all duration-300 border-border/50 hover:border-primary/20 bg-card overflow-hidden">
            <CardHeader className="pb-4">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <CardTitle className="text-xl font-semibold group-hover:text-primary transition-colors">{user.name}</CardTitle>
                    <CardDescription className="text-sm mt-1">{user.packs.length} bộ từ</CardDescription>
                  </div>
                </div>
                ...
                <div className="flex opacity-0 group-hover:opacity-100 transition-opacity">
                  <Button 
                    ...
                    onClick={() => setEditingUser({ oldName: user.name, newName: user.name })}
                    data-testid={`button-edit-user-${user.name}`}
                  >
                  ...
                  <Button 
                    ...
                    data-testid={`button-delete-user-${user.name}`}
                  >
            ...
            <CardContent>
              <Button 
                ...
                data-testid={`button-study-user-${user.name}`}
              >
                Vào học
              </Button>
            </CardContent>
      ...
      <Dialog open={!!editingUser} onOpenChange={(open) => !open && setEditingUser(null)}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleRename} data-testid="button-confirm-rename">Lưu thay đổi</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!deletingUser} onOpenChange={(open) => !open && setDeletingUser(null)}>
        <DialogContent>
          ...
          <p className="py-4 text-muted-foreground">
            Bạn có chắc chắn muốn xóa người dùng <span className="font-semibold text-foreground">{deletingUser}</span>? Tất cả các bộ từ sẽ bị mất và không thể khôi phục.
          </p>
          <DialogFooter>
            <Button variant="outline" onClick={() => setDeletingUser(null)}>Hủy</Button>
            <Button variant="destructive" onClick={handleDelete} data-testid="button-confirm-delete">Xóa người dùng</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
